...
,   setupDDocs: function (cb) {
        ...
        ,   rejectCondition = function (key, conds, exclude) {
            var str = "    if (!doc." + key + " || (";
            conds = isArray(conds) ? conds : [conds];

            str += conds
                    .map(function (it) {
                        return "doc." + key + (exclude ? " === '" : " !== '") + esc(it) + "'";
                    })
                    .join(exclude ? " || " : " && ")
            ;
            return str + ")) return;";
        ...
        for (var view in eventFilter) {
            ...
            if (eventFilter[view].repository) {
                ...
                if (eventFilter[view].ignoreSender)
                    mapFunc.push(rejectCondition("payload.sender", eventFilter[view].ignoreSender, true));
